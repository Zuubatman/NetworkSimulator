...
serverIP = '172.20.10.12'  # Substitua pelo IP deste roteador
...
def update_routing_table(received_message, sender_ip):
    ...
    for route in routes:
        ...
        # Atualiza ou adiciona rota para dest_ip se não for o próprio IP
        if dest_ip != serverIP:
            if dest_ip not in routing_table or metric < routing_table[dest_ip]["metric"]:
                # Nova rota ou rota com métrica menor
                routing_table[dest_ip] = {"metric": metric, "next_hop": sender_ip, "last_update": time.time()}
                updates = True
            elif dest_ip in routing_table and routing_table[dest_ip]["next_hop"] == sender_ip:
                # Atualiza last_update apenas para rotas que passam pelo sender_ip
